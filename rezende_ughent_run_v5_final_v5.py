...
name = (os.path.abspath(os.path.dirname(os.sys.argv[0]))) + os.sep + 'rezende_ughent_ribs_final_5'
...
sec_gap = 40        # security gap between rings and iii-v stack       [um]

taper_l1 = 60       # taper length                                     [um]
taper_w1 = 0.65     # taper final width                                [um]
taper_l2 = 20       # taper length                                     [um]
taper_w2 = 3     # taper final width                                [um]
taper_l3 = 180       # taper length                                     [um]
taper_w3 = 0.300     # taper final width                                [um]
...
wg_w22 = 11          # waveguide cladding width                         [um]
...
final_pos = numpy.array([(2*r1+sec_gap+taper_l1+taper_l2+taper_l3+taper_ii_l)*2+iii_v_l,(n*d)])
...
## ------------------------------------------------------------------ ##
    ...
    waveguide.segment(r1*(1-2*sin(pi/8.0)),'+x',**spec)
    waveguide.segment(sec_gap,'+x',**spec)
    waveguide.segment(taper_l1,'+x',final_width= taper_w2,**spec)
    waveguide.segment(taper_l2,'+x',final_width= taper_w2,**spec)
    waveguide.segment(taper_l3,'+x',final_width= taper_w3,**spec)

    

    #------ No tilling region   -----------------------------------------------#
    nofill = gdspy.Path(nofill_w,(waveguide.x-taper_l1-taper_l2-taper_l3,waveguide.y))
    nofill.segment(taper_l1+taper_l2+taper_l3+taper_ii_l+iii_v_l/2.0,'+x',**spec3)
    

    #------ Coupled Microrings   -----------------------------------------------#
    r2 = (r1-wg_w-3.0*gap/2.0)/2.0 
    pcavc = (waveguide.x-sec_gap-r1-taper_l1-taper_l2-taper_l3,waveguide.y-gap-0.5*wg_w-r1*2*(1.5-cos(pi/8.0)))
    ...
    waveguide2.segment(r1*(1-2*sin(pi/8.0)),'+x',**spec2)
    waveguide2.segment(sec_gap,'+x',**spec2)
    waveguide2.segment(taper_l1,'+x',final_width= wg_w22,**spec2)
    waveguide2.segment(taper_l2,'+x',**spec2)
    waveguide2.segment(taper_l3,'+x',**spec2)
