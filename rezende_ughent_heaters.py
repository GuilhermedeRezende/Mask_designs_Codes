...
def ncmask1(layer_num,center,ang,radius,hw,hh)
    nc1 = gdspy.Round(center,radius+hw/2,radius-hw/2,initial_angle=-3.0*pi/4.0, final_angle=-pi/4.0, layer=layer_num,number_of_points=2000,max_points=199)
    nc2 = gdspy.Rectangle(center[0]-radiu*sin(ang), center[1]-radius*cos(ang), layer=layer_num)
    nc3 = gdspy.Rectangle(center[0]-radiu*sin(ang)-hw, center[1]-radius*cos(ang)-hh, layer=layer_num)
    bool = gdspy.boolean([nc1,nc2],sum,max_points=199,layer=layer_num)
    bool = gdspy.boolean([bool,nc3],sum,max_points=199,layer=layer_num)
    return bool
...
def draw(gap):
    ...
    htnicrcell = gdspy.Cell('Heater_Ni_Cr'+ str(2*n*(gap-gap0)/(gap0+gap1)))
