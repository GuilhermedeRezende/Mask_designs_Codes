...
name = (os.path.abspath(os.path.dirname(os.sys.argv[0]))) + os.sep + 'n_contact_model'

##-------------------------------------------------------------##
##        PARAMETERS
##-------------------------------------------------------------##

n_w = 25        # n-contact width width  [um]
n_ofs = 17        # n-contact offset  [um]
...
waveguiden = gdspy.Path(n_w, (waveguide.x,waveguide.y+n_ofs))
waveguiden.segment(2*(taper_l+taper_ii_l)+iii_v_l,'+x',**spec)

waveguiden2 = gdspy.Path(n_w, (waveguide.x,waveguide.y-n_ofs))
waveguiden2.segment(2*(taper_l+taper_ii_l)+iii_v_l,'+x',**spec)
...
wg_cell.add(waveguiden)
wg_cell.add(waveguiden2)  

waveguidex=waveguiden2.x
